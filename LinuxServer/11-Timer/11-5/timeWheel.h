...
public:
    ...
    // 根据定时器的timeout创建一个定时器，并把它插入合适的槽中
    // 返回timer后可以对应修改数据
    tw_timer* add_timer(int timeout)
    {
        ...
        // 将非固定的timeout转化为固定的滴答时间
        // 根据timeout来折算定时器的超时值将在多少滴答后被触发，将滴答数存于变量ticks中
        // 如果插入定时器的超时值小于时间轮的槽时间SI，则将ticks向上折合为1，否则向下折合
        if(timeout < SI)
        {
            ticks = 1;
        }
        ...
        // 如果第ts个槽上尚无任何定时器，则把新建的定时器插入，设为槽头节点
        if(!slots[ts])
        {
    ...
    void del_timer(tw_timer* timer)
    {
        ...
        // slots[ts]是目标定时器所在槽头结点。
        // 如果目标定时器就是该头结点，则需重置第ts个槽的头结点
        if(timer == slots[ts])
        {
    ...
    void tick()
    {
        ...
        while(tmp)
        {
            ...
            // 如果定时器rotation值大于0，则它在这一轮中不起作用
            if(tmp->rotation > 0)
            {
                tmp->rotation--;
                tmp = tmp->next;
            }
            // 否则定时器已经到期，执行任务后删除该定时器
            else
            {
        ...
        cur_slot = ++cur_slot % N;  // 更新时间轮的当前槽，反应时间轮的转动
    }
